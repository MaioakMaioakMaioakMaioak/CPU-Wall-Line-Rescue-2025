...
void FF(int speed, float timer, char select, int speedTurnLeft, int speedTurnRight, float turnTimer) {
  int min_speed = 15;     // ความเร็วเริ่มต้น และความเร็วก่อนหยุด
  int max_speed = speed;  // ความเร็วสูงสุด
  float kp = 0.8;         // KP
  float kd = 10.0;        // KD
  ...
  if (select == 'L') {
    ...
    TL(speedTurnLeft, speedTurnRight, turnTimer);
  } else if (select == 'r') {
    TR(speedTurnLeft, speedTurnRight, turnTimer);
  }
  delay(delayALL);
}
